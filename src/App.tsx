...
function App() {
  ...
  useEffect(() => {
    ...
    if (loggedInUser && items.length === 0) {
      setCurrentPrNumber(generateUniquePoNumber(loggedInUser));
    }
  }, [loggedInUser, items]);
  ...
  const handleLogin = () => {
    ...
    if (user) {
      ...
      setCurrentPrNumber(generateUniquePoNumber(user.username));
    } else {
  ...
  const handleLogout = () => {
    ...
    setCurrentPrNumber(generateUniquePoNumber(null)); // Generate new PO for next session
  };
  ...
  const handleClearAll = () => {
    if (window.confirm('Are you sure you want to clear all items?')) {
      setItems([]);
      setCurrentPrNumber(generateUniquePoNumber(loggedInUser)); // Generate new PO number on clear
    }
  };
